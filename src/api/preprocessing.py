    ...
    for key, expected_type in expected_types.items():
        if key in data:
            ...
            if value is not None and not isinstance(value, expected_type):
                # Gérer le cas où expected_type est un tuple (ex: (int, float))
                if isinstance(expected_type, tuple):
                    type_names = " ou ".join([t.__name__ for t in expected_type])
                else:
                    type_names = expected_type.__name__
                
                raise ValueError(
                    f"Type incorrect pour {key}: attendu {type_names}, "
